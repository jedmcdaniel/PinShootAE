...
namespace PinShoot
{
    public partial class OtherTimeEntry : Form
    {
        OtherResults _owner;
        float bestTime;
        float tieBreaker;

        public OtherTimeEntry(OtherResults owner)
        {
            _owner = owner;
        ...
        private void OtherTimeEntry_FormClosing(object sender, FormClosingEventArgs e)
        {
            _owner.LoadResults();
        }
        ...
        private void SaveTimeEntry()
        {
            ...
            using (var command = new SqlCommand("SsOther", conn)
            {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            var viewConString = System.Configuration.ConfigurationManager.ConnectionStrings["PinShootDb"].ConnectionString;
            using (var conn = new SqlConnection(viewConString))
            using (var command = new SqlCommand("SxOtherEntry", conn)
            {
                CommandType = CommandType.StoredProcedure,
            })
            {
        ...
        public void LoadEntry(int entryId)
        {
            ...
            using (var conn = new SqlConnection(viewConString))
            {
                cmd.CommandText = "SlOtherResults";
